...
const initEditor = (item: FormOptions) => {
    nextTick(() => {
        if (document.getElementById('editor') && document.getElementById('toolbar')) {
            ...
            editorConfig.placeholder = item.placeholder! //文字提示

            //解决数据，第二次输入 没有获取到值得问题
        ...
        <template v-for="(item, index) in options" :key="index">
            <el-form-item v-if="!item.children || !item.children!.length" :label="item.label" :prop="item.prop">
                ...
                <component v-if="item.type !== 'upload' && item.type !== 'editor'" v-bind="item.attrs"
                    :is="`el-${item.type}`" v-model="model[item.prop!]" :placeholder="item.placeholder"></component>
                ...
                <div class="full-screen-container">
                    <div class="toolbar" id="toolbar" v-if="item.type === 'editor'" style="z-index: 101;"></div>
